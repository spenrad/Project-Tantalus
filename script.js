$(document).ready(function () {
    ...
    $.ajax({
      ...
      for (var i = 0; i < Math.min(10, response.restaurants.length); i++) {
        ...
        var restaurantObject = {
          ...
          rating:
            response.restaurants[randNum].restaurant.user_rating
              .aggregate_rating,
        };
  ...
  function randomRecipe() {
    ...
    $.ajax({
      ...
      while (
        j < 21 &&
        response.meals[0]["strIngredient" + j] != "" &&
        response.meals[0]["strIngredient" + j] != " " &&
        response.meals[0]["strIngredient" + j] != null
      ) {
      ...
      while (
        j < 21 &&
        response.meals[0]["strMeasure" + j] != "" &&
        response.meals[0]["strMeasure" + j] != " " &&
        response.meals[0]["strMeasure" + j] != null
      ) {
  ...
  function recipeSearch(queryType, query) {
    ...
    $.ajax({
      ...
      for (var i = 0; i < idArray.length; i++) {
        queryURL =
          "https://www.themealdb.com/api/json/v1/1/lookup.php?i=" + idArray[i];
        $.ajax({
          url: queryURL,
          method: "GET",
        }).then(function (response) {
